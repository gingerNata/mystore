 ...
 *   id = "taxonomy_last_child_id_from_entity",
 *   title = @Translation("Taxonomy last child term ID from any entity")
 * )
 */
class LastChildTerm extends ArgumentDefaultPluginBase implements CacheableDependencyInterface {
  ...
  public function buildOptionsForm(&$form, FormStateInterface $form_state) {
    ...
    $form['entity'] = [
      '#type' => 'checkbox',
      '#title' => $this->t("Load default filter from entity page, that's good for related taxonomy blocks"),
    ...
    $form['limit'] = [
      ...
      '#states' => [
        'visible' => [
          ':input[name="options[argument_default][taxonomy_last_child_id_from_entity][entity]"]' => ['checked' => TRUE],
        ],
      ],
    ];
    ...
    $form['vids'] = [
      ...
      '#states' => [
        'visible' => [
          ':input[name="options[argument_default][taxonomy_last_child_id_from_entity][limit]"]' => ['checked' => TRUE],
          ':input[name="options[argument_default][taxonomy_last_child_id_from_entity][entity]"]' => ['checked' => TRUE],
        ],
      ],
    ];
  ...
  public function getArgument() {
    ...
    // Load default argument from entity.
    if (!empty($this->options['entity'])) {
      $entities = array_keys(\Drupal::entityTypeManager()->getDefinitions());
      // Just check, if a entity could be detected.
      foreach ($entities as $entity_name){
        if ($entity = $this->routeMatch->getParameter($entity_name)){
          break;
        };
      }
        ...
        foreach ($entity->getFieldDefinitions() as $field) {
          if ($field->getType() == 'entity_reference' && $field->getSetting('target_type') == 'taxonomy_term') {
            ...
            foreach ($taxonomy_terms as $taxonomy_term) {
                if (!$children = $term_storage->loadChildren($taxonomy_term->id())) {
